...
namespace {
bool workspace_is_visible(const PHLWORKSPACE& workspace) {
    if (!workspace)
        return false;
    const auto monitor = workspace->m_monitor.lock();
    return monitor && (monitor->activeWorkspaceID() == workspace->m_id ||
    ...
    : reorder(Reorder::Auto), row(row)
{
    ...
    auto window = std::make_unique<Window>(cwindow, max.y, max.h, width);
...
void Column::add_active_window(PHLWINDOW window)
{
    ...
    auto w = std::make_unique<Window>(window, row->get_max().y, row->get_max().h, wwidth);
    ...
    default:
        node = windows.emplace_after(active, w.get());
        break;
    case ModeModifier::POSITION_BEFORE:
        node = windows.emplace_before(active, w.get());
        break;
    case ModeModifier::POSITION_END:
        node = windows.emplace_after(windows.last(), w.get());
        break;
    case ModeModifier::POSITION_BEGINNING:
        node = windows.emplace_before(windows.first(), w.get());
        break;
    }
    w.release();
...
{
    Log::logger->log(Log::DEBUG, "[SCROLLER] recalculate_col_geometry called, column has {} windows", windows.size());

    if (!active)
        return;

    // Immediately update visibility of admitted windows based on workspace (before any geometry calculations)
    for (auto w = windows.first(); w != nullptr; w = w->next()) {
        if (w != active) {
            auto hyprwin = w->data()->get_window();
            if (hyprwin) {
                auto workspace = hyprwin->m_workspace;
                const bool isActiveWorkspace = workspace_is_visible(workspace);

                if (!isActiveWorkspace) {
                    // Hide if not on the currently active workspace
                    hyprwin->setHidden(true);
                } else {
                    // Show if on the currently active workspace
    ...
    if (row->get_mode_modifier().center_window_enabled() && row->get_active_column() == this) {
...
void Column::store_full_sizes_for_clipping()
{
    // For windows with clip_when_inactive, store their current full size before resizing
    for (auto w = windows.first(); w != nullptr; w = w->next()) {
        Window* window = w->data();
...
void Column::resize_active_window(const Vector2D &gap_x, double gap, const Vector2D &delta)
{
    if (!active) return;
    const Box &max = row->get_max();
    if (delta.x != 0.0 && !active->data()->can_resize_width(geom.w, max.w, gap_x, gap, delta.x))
        return;
...
void Column::adjust_windows(ListNode<Window *> *win, const Vector2D &gap_x, double gap, bool animate)
{
    ...
    // Use cached activeWID for visibility checks in adjust_windows
    for (auto w = windows.first(); w != nullptr; w = w->next()) {
        auto gap0 = w == windows.first() ? 0.0 : gap;
        auto gap1 = w == windows.last() ? 0.0 : gap;

        // Hide admitted windows on non-active workspaces
        auto hyprwin = w->data()->get_window();
        if (hyprwin && windows.size() > 1 && w != active) {
            // Non-active window (admitted window) in a column
            auto workspace = hyprwin->m_workspace;
            const bool isActiveWorkspace = workspace_is_visible(workspace);

            if (!isActiveWorkspace) {
                // Hide admitted windows on non-active workspaces to prevent animation artifacts
                hyprwin->setHidden(true);
            } else {
                // Show admitted windows on the active workspace
                hyprwin->setHidden(false);
            }
        } else if (hyprwin) {
            // Active window or single window - always visible
            hyprwin->setHidden(false);
        }
...
bool Column::has_clipped_window() const
{
    for (auto w = windows.first(); w != nullptr; w = w->next()) {
        auto hyprwin = w->data()->get_window();
        if (hyprwin && hyprwin->m_ruleApplicator &&
            hyprwin->m_ruleApplicator->m_tagKeeper.isTagged("scroller:clip_when_inactive")) {
            return true;
        }
    }
    return false;
...
Column *Column::selection_get(const Row *row)
{
    ...
    PHLWORKSPACE workspace = workspace_by_id(row->get_workspace());
    if (!workspace)
        return nullptr;
    while (win != nullptr) {
        auto next = win->next();
...
// Grid mode support
Window* Column::get_window_at_grid_row(uint32_t grid_row)
{
    for (auto w = windows.first(); w != nullptr; w = w->next()) {
        if (w->data()->get_grid_row() == grid_row) {
            return w->data();
        }
    }
    return nullptr;
...
void Column::recalculate_grid_col_geometry(uint32_t grid_row, const Vector2D &gap_x, double)
{
    const Box &max = row->get_max();

    Log::logger->log(Log::DEBUG, "[GRID] recalculate_grid_col_geometry: target_grid_row={}, windows_count={}", grid_row, windows.size());

    // In grid mode, show window matching grid_row, hide others below screen
    for (auto w = windows.first(); w != nullptr; w = w->next()) {
        PHLWINDOW hyprwin = w->data()->get_window();
        if (!hyprwin) continue;

        Log::logger->log(Log::DEBUG, "[GRID] Window grid_row={}, target={}", w->data()->get_grid_row(), grid_row);

        if (w->data()->get_grid_row() == grid_row) {
            // This window is visible in the current grid row
            hyprwin->setHidden(false);
            ...
            set_window_position(hyprwin, Vector2D(-10000, -10000), true);
            hyprwin->setHidden(true);
        }
    }
}
