...
StandardSize ScrollerSizes::get_next(const std::vector<StandardSize> &sizes, StandardSize size, int step) const
{
    ...
    const int number = static_cast<int>(sizes.size());
    if (g_scrollerConfig.cyclesize_wrap()) {
        const int offset = step % number;
        current = (number + current + offset) % number;
    }
    else {
        current += step;
        if (current < 0)
...
StandardSize ScrollerSizes::get_window_default_height(PHLWINDOW window)
{
    ...
    if (const auto rule_value = window_rule_effects::windowheight(window))
        return get_size_from_string(*rule_value, StandardSize::One);
...
StandardSize ScrollerSizes::get_column_default_width(PHLWINDOW window)
{
    ...
    if (const auto rule_value = window_rule_effects::columnwidth(window))
        return get_size_from_string(*rule_value, StandardSize::OneHalf);
    const auto monitor = window->m_monitor.lock();
    update();
    if (monitor) {
        for (const auto& monitor_data : monitors) {
...
{
    const auto window_default_height_str = g_scrollerConfig.window_default_height();
    const auto column_default_width_str = g_scrollerConfig.column_default_width();
    const auto window_heights_str = g_scrollerConfig.window_heights();
    const auto column_widths_str = g_scrollerConfig.column_widths();
    const auto monitor_modes_str = g_scrollerConfig.monitor_options();
