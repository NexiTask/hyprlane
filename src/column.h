...
public:
    ...
    Vector2D get_height() const {
        if (windows.empty() || !windows.first()) {
    ...
    void store_full_sizes_for_clipping();  // Store current size for windows with clip_when_inactive
    ...
    // Check if this column contains any window with clip_when_inactive
    bool has_clipped_window() const;

    // Grid mode support
    Window* get_window_at_grid_row(uint32_t grid_row);
    void recalculate_grid_col_geometry(uint32_t grid_row, const Vector2D &gap_x, double gap);
    void set_hidden(bool h) { hidden = h; }
    bool is_hidden() const { return hidden; }
    ListNode<Window *> *windows_first() { return windows.first(); }
...
private:
    ...
    bool hidden = false;  // Grid mode: column is hidden when it has no window in current grid row
};
