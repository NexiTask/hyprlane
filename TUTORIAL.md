...
Hyprlane is a [Hyprland](https://github.com/hyprwm/Hyprland)
...
*hyprlane* supports multiple workspaces per monitor. Each workspace is a
...
`onefourth` or `onesixth` to fit 3, 4 or 6 windows in the viewport at a time.
...
When you change window focus, *hyprlane* will always show the active
...
1. Assign a key binding to the `jump` action. Lua uses the action suffix with
   the plugin API:

```lua
hl.bind(mainMod .. " + slash", function()
  hl.plugin.scroller.dispatch("jump")
end)
```
...
*hyprlane* supports vim-like marks. You can use marks to quickly navigate
...
client. When you invoke `marksvisit`, *hyprlane* will take you to that window,
even warping to a different workspace if necessary.

```lua
-- Marks are dispatcher actions, not window-rule effects. Assign the mark from
-- a binding while the target window is active.
hl.bind(mainMod .. " + M", function()
  hl.plugin.scroller.dispatch("marksadd", "m")
end)
hl.bind(mainMod .. " + apostrophe", function()
  hl.plugin.scroller.dispatch("marksvisit", "m")
end)
```
...
By default, *hyprlane* supports three and four finger touchpad swipe
